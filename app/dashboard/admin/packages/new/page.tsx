...
export default async function NewPackagePage() {
  const user = await requireUser("/dashboard/admin/packages/new");
  ...
  async function savePackage(formData: FormData) {
    ...
    const currentUser = await requireUser("/dashboard/admin/packages/new");
    ...
    const savedPackage = await upsertPackage({
      id,
    ...
    revalidatePath("/");
    revalidatePath(`/packages/${savedPackage.id}`);
  ...
  return (
    <AdminPage className="max-w-5xl">
      <AdminHeader
        title="Add Package"
        description="Create a complete package profile including itinerary, pricing, and inclusions for booking consistency."
        actions={
          <Link
      ...
      <form action={savePackage} className="space-y-4">
        <AdminPanel title="Core Package Details" description="These values appear in package cards and booking forms.">
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-sm">
              <span className="mb-1.5 block text-muted-foreground">Package ID (slug)</span>
            ...
            <label className="text-sm">
              ...
              <Input name="image" placeholder="/gallery/image8.jpeg" className={adminInputClass} />
            </label>
            <label className="text-sm">
            ...
            <label className="text-sm">
              ...
              <Input name="altitude" required className={adminInputClass} />
            </label>
            <label className="text-sm">
          ...
          <label className="mt-3 block text-sm">
            ...
            <Input name="idealFor" required className={adminInputClass} />
          </label>
        ...
        <AdminPanel
          ...
          description="Use one item per line to keep parsing clean and editable later."
        >
          <label className="block text-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <label className="block text-sm">
                  ...
                  <Textarea name="includesLines" required rows={8} className={adminCodeTextareaClass} />
                </label>
                <label className="block text-sm">
                  <span className="mb-1.5 block text-muted-foreground">Excludes (one line each)</span>
                  <Textarea name="excludesLines" required rows={8} className={adminCodeTextareaClass} />
                </label>
              </div>
            </div>
